...
def load_and_merge_data(params_path, stats_path, params_to_keep):
    ...
    # 3. Merge
    df_merged = pd.merge(df_stats, df_pivot, on=['pid'], how='inner')
    
    # 4. Filter for Baseline
    # df_baseline = df_merged[df_merged['year'] == 'baseline'].copy()
    df_baseline = df_merged.copy() # --- Keep all years for distribution plotting ---
...
def main():
    try:
        ...
        # --- Input Files ---
        PARAMS_CSV = OUT_DIR / "params_posteriors.csv"
        STATS_CSV = "/Users/grahamadmin/Documents/context_exploration/expt/data/complete/expt_3/df.csv"
        STATS_CSV = Path(STATS_CSV)

        # --- Output ---
        OUTPUT_PLOT = FIG_DIR / "fig_param_dist.png"
